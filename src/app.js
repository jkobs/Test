...
(function () {
  ...
  // Resolve with the first fulfilled promise; reject only if all reject.
  // (Promise.any isn't available on some older mobile browsers.)
  function _promiseAny(promises) {
    return new Promise(function(resolve, reject) {
      var pending = promises.length;
      if (!pending) { reject(new Error('no promises')); return; }
      var settled = false;
  ...
  // Race ALL Overpass mirrors in parallel; first valid response wins. This is
  // far more reliable than sequential fallback when the busiest mirror is slow.
  function overpassFetch(query, _unused, timeoutMs) {
    timeoutMs = timeoutMs || 15000;
    if (typeof fetch === 'undefined') return Promise.reject(new Error('no fetch'));
    var attempts = OVERPASS_MIRRORS.map(function(url) {
      ...
      return fetch(url, {
        ...
        // guard against a mirror returning an error doc / non-element payload
