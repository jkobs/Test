...
(function () {
  ...
  function fetchNearestLake(lat, lng) {
    ...
    // out bb returns each element's bounding box + tags cheaply (no full geometry),
    // so even Lake Superior is light. Only rivers (not streams) and bays/lakes.
    ...
    overpassFetch(q).then(function(j) {
      ...
      (j.elements || []).forEach(function(e) {
        ...
        if (b) {
          ...
          // focus point = nearest point of the water body to the user
        ...
        // size filter: lakes must be reasonably large; rivers & bays always pass
        if (!isRiver && !isBay) {
        ...
        if (existing) {
          if (dist < existing.dist) { existing.dist = dist; existing.lat = wlat; existing.lng = wlng; }
          if (size > existing.size) existing.size = size;
        } else {
          waters.push({ name: e.tags.name, lat: wlat, lng: wlng, dist: dist, type: type, size: size });
        }
      });
      // sort by distance, then by size (larger first) for ties at 0 mi
      waters.sort(function(a, b2) { return (a.dist - b2.dist) || (b2.size - a.size); });
