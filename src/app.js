...
(function () {
  ...
  function searchLakeByName(query) {
    ...
    fetchJson(buildUrl(primaryWhere), LAKE_SEARCH_TIMEOUT).then(function(fc) {
      ...
      // concluding there's no such lake (handles case quirks / UPPER rejection).
      return fetchJson(buildUrl(fallbackWhere), LAKE_SEARCH_TIMEOUT).then(function(fc2) { render(parseResults(fc2)); });
    }).catch(function() {
      // Primary errored — retry with the fallback form before giving up.
      fetchJson(buildUrl(fallbackWhere), LAKE_SEARCH_TIMEOUT).then(function(fc2) { render(parseResults(fc2)); })
        .catch(function() {
